...
var global=typeof self !== "undefined" ? self : typeof window !== "undefined" ? window : {};;__browserify_shim_require__=require;(function browserifyShim(module, exports, require, define, browserify_shim__define__module__export__) {
...
var $, ClusterConfigInstanceCtrl, DataSourceDisableInstanceCtrl, DataSourceEnableInstanceCtrl, RuleEditorInstanceCtrl, app, d3, moment, startTime, _,
  __slice = [].slice,
...
app.factory('$druid', function($q, $http, $utils, $window) {
  ...
  return {
    ...
    getAndProcess: function() {
      ...
      $http.get(this.proxy(url)).success(function(data) {
        cbArgs.unshift(data);
        return deferred.resolve(cb.apply($utils, cbArgs));
      });
    ...
    getNodes: function() {
      return this.getAndProcess("/servers?simple", $utils.processServers);
    },
    ...
    getDataSource: function(dataSourceId) {
      return this.getAndProcess("/datasources/" + dataSourceId, $utils.processDataSource);
    },
    getLoadStatus: function(dataSources) {
      return this.getAndProcess("/loadstatus", $utils.processLoadStatus, dataSources);
    },
    getLoadQueue: function(tiers) {
      return this.getAndProcess("/loadqueue?simple", $utils.processLoadQueue, tiers);
    },
    ...
    getTierIntervals: function(tier, dataSources) {
      return this.getAndProcess("/tiers/" + tier + "?simple", $utils.processTierIntervals, tier, dataSources);
    },
    getDataSourceIntervals: function(dataSourceId) {
      return this.getAndProcess("/datasources/" + dataSourceId + "/intervals?simple", $utils.processDataSourceIntervals);
    },
    getSegmentsForInterval: function(dataSourceId, interval) {
      var cleanInterval;
      cleanInterval = interval.replace('/', '_');
      return this.getAndProcess("/datasources/" + dataSourceId + "/intervals/" + cleanInterval + "?full", $utils.processSegmentsForInterval);
    },
    getAllRules: function(dataSources) {
      return this.getAndProcess("/rules", $utils.processAllRules, dataSources);
    },
    getRules: function(dataSourceId) {
      return this.getAndProcess("/rules/" + dataSourceId, $utils.processRules);
    },
    saveRules: function(dataSourceId, rules) {
      var deferred;
      deferred = $q.defer();
      $http.post(this.proxy("/rules/" + dataSourceId), rules).success(function() {
        return deferred.resolve();
      }).error(function(data, status, headers) {
  ...
  return {
    ...
    processTierIntervals: function(results, tier, dataSources) {
      ...
      for (ds in results) {
        ...
        og = _.find(dataSources, function(d) {
          return d.id === ds;
        });
        if (og.tiers == null) {
          og.tiers = {};
...
app.directive('siteNav', function() {
  return {
    ...
    controller: function($scope, $druid) {
      return $druid.getCoordinator().then(function(c) {
...
app.directive('isoDuration', function() {
  return {
    require: 'ngModel',
...
app.directive('isoInterval', function() {
  return {
    require: 'ngModel',
    link: function($scope, $el, $attrs, $ctrl) {
      return $ctrl.$parsers.unshift(function(viewValue) {
...
app.directive('selectTextOnClick', function() {
  return {
    restrict: 'A',
...
app.directive('tierCapacity', function() {
  return {
    restrict: 'E',
    link: function($scope, el) {
      var barHeight, barWidth, bottomLabels, capDiv, capacityG, currSize, growth, labels, minResilience, mmx, nodes, nodesG, percentFormat, siBytesFormat, svg, tier, tierData, topLabels, topMargin, _ref;
      ...
      topLabels.append('div').attr('class', 'maxSize').html(function(d) {
        return "<span class=\"value\">" + (siBytesFormat(d.maxSize)) + "</span> total";
      });
      currSize = topLabels.append('div').attr('class', 'currSize');
      currSize.append('span').attr('class', 'size  value').text(function(d) {
        return siBytesFormat(d.currSize);
      });
      currSize.append('span').attr('class', 'percent').text(function(d) {
        return " (" + (percentFormat(d.utilization)) + ") ";
      });
      currSize.append('span').attr('class', 'label').text('used');
...
app.directive('tierNodes', function() {
  return {
    restrict: 'E',
    link: function($scope, el) {
      ...
      x = d3.scale.linear().domain([0, d3.max(_.pluck($scope.tier.nodes, 'maxSize'))]).range([0, barWidth]);
...
app.directive('rulesTimeline', function($window, $filter, $compile) {
  return {
    restrict: 'A',
    link: function($scope, $el) {
      var draw;
      draw = function() {
        ...
        return console.log("Drew " + $scope.dataSource.id + " after " + ((new Date() - startTime) / 1000) + "s");
      };
...
app.directive('timeline', function($window, $filter, $compile) {
  return {
    restrict: 'A',
    link: function($scope, $el) {
      var draw;
...
app.controller('DruidClusterCtrl', function($scope, $druid, $q) {
  $scope.env = $druid.env;
  $druid.getNodes().then(function(cluster) {
    $scope.tiers = cluster.tiers;
    return $q.all([$druid.getLoadQueue($scope.tiers)]);
  });
  return $druid.getDataSources().then(function(dataSources) {
    $scope.dataSources = dataSources;
    return $druid.getLoadStatus($scope.dataSources);
  });
});

app.controller('DruidDataSourcesCtrl', function($scope, $druid, $q) {
  $scope.env = $druid.env;
  $scope.loadFootprints = function(show) {
    if (!$scope.tiers) {
      $scope.footprintsLoading = true;
      return $druid.getTierNames().then(function(tierNames) {
        var t, tiers, _i, _len;
  ...
  $scope.setEnabled = function(status) {
    ...
    if (status === 'disabled' && ($scope.disabledDataSources == null)) {
      return $druid.getAllDataSources().then(function(dataSources) {
  ...
  $druid.getDataSources().then(function(dataSources) {
    $scope.dataSources = dataSources;
    return $q.all({
...
app.controller('DruidDataSourceCtrl', function($scope, $druid, $stateParams, $q) {
  $scope.env = $druid.env;
  $scope.dataSource = {
    id: $stateParams.id
  };
  $scope.segUrl = function(segId) {
    return $druid.proxy("/datasources/" + $scope.dataSource.id + "/segments/" + segId);
  };
  $scope.selectInterval = function(interval) {
    ...
    if ($scope.serverTiers == null) {
      $druid.getServerTiers().then(function(serverTiers) {
        return $scope.serverTiers = serverTiers;
      });
    }
    return $druid.getSegmentsForInterval($scope.dataSource.id, $scope.selectedInterval.interval).then(function(segments) {
  ...
  $druid.getDataSource($scope.dataSource.id).then(function(dataSource) {
    return angular.extend($scope.dataSource, dataSource);
  });
  $druid.getRules($scope.dataSource.id).then(function(rules) {
    $scope.dataSource.rules = rules;
    return $druid.getDataSourceIntervals($scope.dataSource.id);
  }).then(function(results) {
  ...
  $druid.getRules('_default').then(function(defaultRules) {
    var rule, _i, _len;
...
app.controller('RuleEditorCtrl', function($scope, $modal, $druid, $utils) {
...
RuleEditorInstanceCtrl = function($scope, $modalInstance, originalRules, dataSourceId, tierNames, $druid) {
  ...
  $scope.replicantsNeeded = function(rule) {
    return rule.direction === 'load' && _.isEmpty(rule.tieredReplicants);
  };
  ...
  $scope.save = function() {
    ...
    $druid.saveRules($scope.dataSourceId, rulesToSave).then((function() {
...
app.controller('DataSourceDisableCtrl', function($scope, $modal, $druid, $utils, $location) {
  return $scope.open = function() {
    var modalInstance;
    modalInstance = $modal.open({
...
DataSourceDisableInstanceCtrl = function($scope, $modalInstance, dataSourceId, $druid) {
  $scope.dataSourceId = dataSourceId;
  $scope.confirm = function() {
    console.log("disabling " + $scope.dataSourceId);
    $scope.loading = true;
    $druid.disableDataSource($scope.dataSourceId).then((function() {
...
app.controller('DataSourceEnableCtrl', function($scope, $modal, $druid, $utils, $location) {
  return $scope.open = function() {
    var modalInstance;
    modalInstance = $modal.open({
...
DataSourceEnableInstanceCtrl = function($scope, $modalInstance, dataSourceId, $druid) {
  $scope.dataSourceId = dataSourceId;
  $scope.confirm = function() {
    console.log("enabling " + $scope.dataSourceId);
    $scope.loading = true;
    $druid.enableDataSource($scope.dataSourceId).then((function() {
      return $modalInstance.close();
    }), (function(reason) {
      return $scope.postError = reason;
    }));
...
app.controller('ClusterConfigCtrl', function($scope, $modal, $druid, $utils, $location) {
  return $scope.open = function() {
    var modalInstance;
    modalInstance = $modal.open({
...
ClusterConfigInstanceCtrl = function($scope, $modalInstance, $druid) {
  $scope.loading = true;
  $druid.getClusterConfig().then(function(config) {
    $scope.config = config;
  ...
  $scope.save = function() {
    ...
    if (confirm("Do you really want to update the cluster configuration?")) {
      ...
      $druid.saveClusterConfig($scope.config).then((function() {
        return $modalInstance.close();
      }), (function(reason) {
